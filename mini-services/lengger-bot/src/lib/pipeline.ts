...
export async function runDailyPipeline(
  ...
  // 4c. Add metadata header at the top (source URL + title + author + capture time)
  // Format: comment lines starting with ";" (parser ignores them) + a clickable link line
  const capturedAt = new Date().toISOString();
  const metadataHeader = [
    `; ── Metadata (auto-generated by pipeline) ──`,
    `; Title: ${post.title}`,
    `; Sumber URL: ${post.postUrl}`,
    `; Author: ${post.authorName}`,
    `; Tanggal capture: ${capturedAt}`,
    `; Foto: ${photoFiles.join(", ")}`,
    `; ────────────────────────────────────────────`,
    ``,
  ].join("\n");
  finalText = metadataHeader + finalText;

  // 5. Save raw md (the LLM-fixed + cleaned version with metadata header)
